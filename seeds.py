...
def process_ranges(func, values):
    for v in values:
        if v == 'all':
            for i in range(1, len(seed_data)):
                func(seed_data[i], i)
        elif '-' in v:
            r = v.split('-')
            start = int(r[0])
...
a.add_argument('command', choices=['print', 'get_all', 'get_rules', 'get_abolish', 'get_adopt', 'get_table'], help='command')
...
if args.command == 'print':
    ...
    process_ranges(print, args.ranges)

# Get rules allows us to see the rule names.
elif args.command == 'get_rules':
    process_ranges(lambda x : print(rules.get_rule(x)), args.ranges)

# Get rules allows us to see the rule names.
elif args.command == 'get_abolish':
    process_ranges(lambda x : print('yes' if rules.can_abolish(x) else 'no'), args.ranges)

# Get rules allows us to see the rule names.
elif args.command == 'get_adopt':
    process_ranges(lambda x : print('yes' if rules.can_abolish(x) else 'no'), args.ranges)

elif args.command == 'get_table':
    seed_list = []
    value_list = []
    rule_list = []
    abolish_list = []
